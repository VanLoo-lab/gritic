...
def validate_interval_width(width):
    """Return a credible-interval width expressed as a percentage."""
    if (
        ...
        raise ValueError(
            'interval width must be a finite number greater than 0 and at '
            'most 100'
        )
    return float(width)
...
class TimingIntervalConfig:
    ...
    route_gain: IntervalSpec = field(
        default_factory=lambda: IntervalSpec(95.0)
    )
    tree_gain: IntervalSpec = field(
        default_factory=lambda: IntervalSpec(90.0)
    )
...
def get_interval_bounds(samples, interval):
    ...
    if interval.method == EQUAL_TAILED:
        tail_width = (100.0 - interval.width) / 2.0
        low, high = np.percentile(
            values,
            [tail_width, 100.0 - tail_width],
        )
        return float(low), float(high)

    ordered_values = np.sort(values)
    included_count = int(np.ceil(
        interval.width / 100.0 * ordered_values.size
    ))
