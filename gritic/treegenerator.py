...
def convert_to_plotting_tree(route_tree,wgd_timing,route_timing_summary,long_node_order,tol=1e-7,print_out=False):
    ...
    for node in route_tree.nodes:
        ...
        if route_tree.nodes[node]['WGD_Symbol']:
            wgd_in_path = True
        current_timing = route_timing_summary[node]
        ...
        if not wgd_in_path:
            ...
            if len(predecessors)==0:
                ...
                prev_timing = route_timing_summary[long_node_order.index(predecessor)]
...
def get_wgd_trees(tree,wgd_trees_status):
    ...
    for node_length in range(len(internal_nodes)+1):
        for wgd_nodes in itertools.combinations(internal_nodes,node_length):
            ...
            wgd_hash = nx.weisfeiler_lehman_graph_hash(wgd_tree,node_attr='WGD_Symbol')
...
def get_nx_trees(major_cn,minor_cn,wgd_status,wgd_trees_status):
    ...
    for tree in full_trees:
        ...
        tree_store[tree_hash] = tree
    
    return tree_store


def get_possible_paths_recursive(tree,current_node,path,long_nodes):
    if long_nodes and len(tree.out_edges(current_node))==0:
        return [path]
    if not long_nodes and len(nx.descendants(tree,current_node))==2:
        return [path]
    ...
    for edge in tree.out_edges(current_node):
        ...
        possible_paths.extend(get_possible_paths_recursive(tree,edge[1],new_path,long_nodes))
    
    return possible_paths
#https://stackoverflow.com/questions/4122390/getting-the-root-head-of-a-digraph-in-networkx-python
def get_possible_paths(tree,long_nodes):
