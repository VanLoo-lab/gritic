...
- ```--no-merge-adjacent-segments``` Preserve neighboring copy-number segments as separate intervals. By default, GRITIC merges consecutive segments on the same chromosome having identical ```Major_CN``` and ```Minor_CN```, regardless of the gap between them.
- ```--max-merge-gap``` Set the maximum gap in bases between consecutive equal-copy-number segments that may be merged. The threshold is inclusive. If omitted, there is no maximum; use ```0``` to merge only intervals that touch.
...
GRITIC orders copy-number rows by natural chromosome order, ```Segment_Start```, and ```Segment_End``` immediately after validating the input coordinates. Half-open intervals on the same chromosome must not overlap. By default, consecutive rows on the same chromosome with equal ```Major_CN``` and ```Minor_CN``` are merged regardless of the gap between them. The merged interval runs from the first segment's start through the last segment's end, including any intervening uncovered bases. Supplying ```--max-merge-gap N``` (or ```max_merge_gap=N``` through the API) limits each consecutive source-segment gap in a merged run to at most ```N``` bases; the limit is inclusive, and ```0``` requires the intervals to touch. Supplying ```--no-merge-adjacent-segments``` (or ```merge_cn=False``` through the API) instead preserves each input copy-number segment. Final segment IDs are generated from ```Chromosome```, ```Segment_Start```, and ```Segment_End``` after applying the selected merge behavior.
...
The gain-draw and route-ledger data frames used to calculate these summaries are internal. GRITIC no longer writes the legacy ```_posterior_timing_table_penalty_<True|False>.tsv``` or ```_posterior_route_draw_table_penalty_<True|False>.tsv``` files. For routes represented by particles, the original aligned route-conditional ```Timing``` and ```Mult``` particles remain available in the timing stores.

### _route_table.tsv
This table contains exactly one row for each possible route of each timed segment. Its first three columns and key are ```(Sample_ID, Segment_ID, Route)```. ```Timing_Representation``` is ```Route_Particles``` for ordinary gained routes and ```Uniform_No_Gain``` when every extant copy spans the complete ```[0,1]``` interval. It stores both the ordinary ```Probability``` and post-hoc ```Penalized_Probability```, average event and loss counts, timing-geometry proposal density and runtime, and the segment and WGD metadata. The density diagnostic covers the hit-and-run timing coordinates; independently sampled Dirichlet clone shares are not chain dimensions. Analytic uniform routes require no chain and report density 1. Routes with no independently timeable gains are retained here. Its ```WGD_Timing``` interval fields exactly repeat the configured sample-level WGD summary written to the calling-info JSON.
...
When constructing posterior-summary draws, GRITIC reads the authoritative route-probability scalars from the timing archive in route-table order and verifies that they match the corresponding ```_route_table.tsv``` values. It then selects one posterior-array index for the chosen route and uses the same index for the WGD and every independent-gain timing, preserving their joint dependence. The gain timing table supplies node identity and phasing metadata; the full aligned posterior arrays remain in ```_timing_dicts```.
...
### _timing_dicts
Compressed timing stores containing the posterior arrays needed by downstream mutation timing. ```Route_Particles``` segments store gain timing and multiplicity particles. A ```Uniform_No_Gain``` segment writes no store when it has no subclones; with subclones it writes only the fitted clone-share posterior. These stores are not necessary for most use cases. Each logical store that exists is a required pair:
...
The reconstructed dictionary keys are the opaque route identifiers. Every ```Route_Particles``` entry is self-describing and contains exactly these numeric arrays:

- ```Probability``` and ```Penalized_Probability``` are one-element float64 arrays. They are the authoritative route weights used for mutation timing and exactly match the two route-table probability columns.
- ```Timing``` is a float64 ```N_Particles x N_Timing_Nodes``` matrix. ```Timing_Node_ID``` is the aligned int64 structural-node identifier for each column. These are structural IDs, not chronological ```Gain_Index``` values.
...
- ```State_Column_Offsets``` and ```State_Columns``` map the three phasings, in ```non_phased```, ```major```, ```minor``` order, onto ```Mult``` columns. The four offsets delimit the three slices in the flattened column vector. Shared subclone columns occur at the tail of every applicable slice.
...
- ```Archive_Kind``` is 0 for an ordinary segment store and 1 for a pooled-WGD store. Pooled ```2+2``` output explicitly records target ```2+2``` WGD metadata and model ```2+0``` non-WGD metadata; every phasing mask and state slice points to the representative major-allele geometry.

A subclonal ```Uniform_No_Gain``` route instead contains only ```Clone_Share```, whose columns are the clonal share followed by each subclone share. No credible interval is applied to any timing store; consumers choose their own interval width and method from the particles.

Each particle payload contains the original 1,000 likelihood-resampled route-conditional particles produced during fitting. Serialization does not resample or duplicate these particles. Rows remain aligned across every array in a route payload, so consumers must use the same row index for all values in one joint posterior draw.

The ```Mult``` blocks retain their fitted order: unphased multiplicities, major-phased multiplicities, minor-phased multiplicities, then shared subclone proportions. Consumers should use ```State_Column_Offsets``` and ```State_Columns``` rather than recreate these offsets from copy number.

Version 4 replaces the former nested node-timing mapping with this compact self-describing route schema and embeds both route-probability families. It retains version 3's analytic ```Uniform_No_Gain``` representation and compact optional ```Clone_Share``` payload. There is no legacy archive reader: regenerate the complete GRITIC sample output before using it with current downstream tools, and never mix route, mutation, gain-timing, or timing-store files from different runs.
