...
The reconstructed dictionary keys correspond to the routes for the sample. A ```Route_Particles``` route contains exactly ```Timing``` and ```Mult```. The ```Timing``` entry gives the stored timing distributions for independent gains indexed by the corresponding structural node in the tree; it does not store a chronological ```Gain_Index```. A WGD timing entry is also given if applicable. No credible interval is applied to timing stores: consumers choose their own interval width and method from the stored samples. A subclonal ```Uniform_No_Gain``` route instead contains only ```Clone_Share```, whose columns are the clonal share followed by each subclone share.
...
Version 3 adds the analytic ```Uniform_No_Gain``` representation and its compact optional ```Clone_Share``` payload. It also retains the version-2 removal of the former proposal snapshot. Existing timing stores must not be mixed with current route, mutation, or gain timing tables; regenerate the complete sample output together.
